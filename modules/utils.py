...
def is_spammer(message, moderator_roles):
    try:
        ...
        return False
        print(f"Error during execution of `is_spammer` function. Message: {message}")
...
def reset_info_file(players):
    data = {
        "players": players,
        "new_runs": {"First Map": [], "Gym Map": [], "Mantle Jump Map": [], "It Hurts Map": [], "Strafe It Map": []},
    }
...
def table_constructor_total_runs(rank, player_name, run_amount):
    table = "```ansi\n"
    table += (
        "[2;45m[2;37m{:<5} {:<20}  {:<10}[0m[2;45m[0m[2;45m[2;37m[0m[2;45m[0m\n".format(
            "Rank", "Username", "Total Runs"
        )
    )
    table += "[2;45m[2;37m[2;47m[2;30m{:<5} {:<26} {:05}[0m[2;37m[2;47m[0m[2;37m[2;45m[0m[2;45m[0m[2;45m[2;37m[0m[2;45m[0m\n".format(
        rank, player_name[:20], run_amount
    )
    table += "```"
...
For now I  can:
* Provide up-to-date information about the **Movement HUB** R5 Reloaded server
* Help with banning spammers that uses `@everyone` and `@here`
* Help with providing better embeds for twitter and reddit links
