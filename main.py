...
from modules.dbIntegration import read_leaderboard, read_personal_best, table_constructor
...
:warning: The bot can take up to 1 minute to update with new info, be patient!"""
        ...
        try:
            leaderboard_firstmap_message = await channel.fetch_message(LEADERBOARD_FIRSTMAP_ID)
            answer = f"""### First Map
{fm}
...
post: https://discord.com/channels/839992880293478400/1180733536676880396"""
        ...
        try:
            leaderboard_mantlejumpmap_message = await channel.fetch_message(LEADERBOARD_MANTLEJUMPMAP_ID)
...
@tree.command(name="leaderboard", description="Returns the top 10 scoreboard for a specific movement map.")
async def self(interaction: discord.Interaction):
     ...
     match channel_name:
         ...
         case _:
            await interaction.response.send_message(f"**{channel_name}** doesn't have a leaderboard.")
            return
...
async def self(interaction: discord.Interaction):
    await interaction.response.send_message(f"Hello! I'm a bot made by loy_ and my purpose is to serve Treeree's server! For more info send /help")
...
@tree.command(name="personal_best", description="Returns 10 quickest times of a specific player for a specific movement map.")
